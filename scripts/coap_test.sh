...
if [ -n "$IOT_LAB_FRONTEND_FQDN" ]; then
  ...
  coap_client_job_json=$(iotlab-experiment submit -n ${COAP_CLIENT_EXE_NAME} -d ${EXPERIMENT_TIME} -l ${SENSE_SITE},m3,${COAP_CLIENT_NODE},${SENSE_FIRMWARE_HOME}/${COAP_CLIENT_EXE_NAME}.elf)
  ...
  echo "nc m3-${COAP_CLIENT_NODE} 20000"
