...
if [ -n "$IOT_LAB_FRONTEND_FQDN" ]; then
  ...
  n_json=$(iotlab-experiment submit -n ${SENSOR_READ_EXE_NAME} -d ${EXPERIMENT_TIME} -l ${SENSE_SITE},m3,${SENSOR_NODE},${SENSE_FIRMWARE_HOME}/${SENSOR_READ_EXE_NAME}.elf,group12)
