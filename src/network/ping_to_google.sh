...
if [ -n "$IOT_LAB_FRONTEND_FQDN" ]; then
  ...
  echo "Submit job to node 28"
  border_router_job_json=$(iotlab-experiment submit -n n2n-border-router-gp12 -d 30 -l grenoble,m3,$NODE_1,~/shared/gnrc_border_router.elf)
  ...
  echo "Submit job to node 26"
  n_json=$(iotlab-experiment submit -n n2n-networking-node-gp12 -d 30 -l grenoble,m3,$NODE_2,~/shared/gnrc_networking.elf)
  n_node_job_id=$(echo $n_json | jq '.id')

  # create a file to stop the experiments
  echo "Creating 'stopper.sh' script"
  touch stopper.sh
  echo "iotlab-experiment stop -i $n_node_job_id" > stopper.sh
  echo "iotlab-experiment stop -i $border_router_job_id" >> stopper.sh
  ...
  sudo ethos_uhcpd.py m3-$NODE_1 tap7 2001:660:5307:3107::1/64 &
